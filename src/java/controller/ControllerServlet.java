...
loadOnStartup = 1,
urlPatterns = {"/index", "/inscription", "/inscriptionValidation", "/connect",
    "", "/deconnect", "/index.html",
    "/pays", "/historique", "/paysAlphabet", "/afficherRecherche",
    "/recherche", "/listePays", "/nouvelleDestination",
    "/dernieresDestinations", "/nouveauPays", "/modifierPays",
    "/uploadFichier", "/downloadFile", "/myProfile", "/xProfile", "/listeProfils",
    "/rechercheResult", "/changeInfoPerso", "/addDestination"})
...
//            }
            ...
            int id = (Integer) request.getSession(false).getAttribute("idProfil");
            ...
            List<Destination> listDest = profilFacade.getDestinationsList(id);
            /*
             * if ((listDest==null)||(listDest.size() == 0)||(listDest.get(0) ==
             * null)){ List<String> listeVille = new ArrayList<String>();
             * List<String> listePays = new ArrayList<String>(); for (int i=0;
             * i<listDest.size(); i++){ entity.Ville ville =
             * destinationFacade.findByIdprofil(id).get(0).getDestinationidDestination();
            ...
            request.getSession(false).setAttribute("nom", name);
            request.getSession(false).setAttribute("prenom", firstName);
            request.getSession(false).setAttribute("promo", promo);
             ...
             * jourd = Integer.parseInt(request.getParameter("jourd")); int ana
             * = Integer.parseInt(request.getParameter("ana")); int moisa =
             * Integer.parseInt(request.getParameter("moisa")); // int joura = Integer.parseInt(request.getParameter("joura"));
             */
            ...
            /*
             * dated.setYear(and); dated.setMonth(moisd); //
             * dated.setDay(jourd); datea.setYear(2000); datea.setMonth(04); //
             * datea.setDay(joura);
             */
            ...
            destinationManager.createDestination(idVille, idPays, 
                      (Integer)request.getSession().getAttribute("idProfil")
                     , type, "organ", com, dateD, dateA);
